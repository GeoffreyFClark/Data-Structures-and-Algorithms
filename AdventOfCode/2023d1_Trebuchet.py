...
calibration_doc = """51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
total_sum = sum_calibration_values(calibration_doc)
print(f"The sum of the calibration values is: {total_sum}")  # 55712
...
# Digit map (Added letters to address truncated keys in data)
digit_map = {"one": "o1e", "two": "t2o", "three": "t3e", "four": "f4r",
             "five": "f5e", "six": "s6x", "seven": "s7n", "eight": "e8t", "nine": "n9e"}

def extract_spelled_digits(line):
    # replace spelled out digits using map
...
def sum_calibration_values(data):
    lines = data.strip().split('\n')
    return sum(extract_spelled_digits(line) for line in lines)

total_sum = sum_calibration_values(calibration_doc)
print(f"The sum of the calibration values is: {total_sum}")  # 55413
