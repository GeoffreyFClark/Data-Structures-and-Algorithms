...
def is_game_possible(subsets, red_limit, green_limit, blue_limit):
    for subset in subsets:
        if subset['red'] > red_limit or subset['green'] > green_limit or subset['blue'] > blue_limit:
            return False
    return True

def parse_game_data(game_line):
    game_id, details = game_line.split(':')
    ...
    for subset in subsets:
        ...
        parsed_subsets.append(color_counts)
    return parsed_subsets

possible_games = []
for line in puzzle_input.strip().split('\n'):
    game_id = int(line.split()[1].strip(':'))
    game_subsets = parse_game_data(line)
    if is_game_possible(game_subsets, RED_LIMIT, GREEN_LIMIT, BLUE_LIMIT):
        possible_games.append(game_id)

sum_of_possible_game_ids = sum(possible_games)
print(f"The sum of the IDs of the possible games is: {sum_of_possible_game_ids}")
...
def calculate_minimum_cubes(game_data):
    ...
    for subset in game_data:
        min_red = max(min_red, subset.get('red', 0))
        min_green = max(min_green, subset.get('green', 0))
        min_blue = max(min_blue, subset.get('blue', 0))
...
for line in puzzle_input.strip().split('\n'):
    game_subsets = parse_game_data(line)
    min_red, min_green, min_blue = calculate_minimum_cubes(game_subsets)
    power = min_red * min_green * min_blue
    total_power_sum += power

print(f"The sum of the power of the minimum sets of cubes is: {total_power_sum}")
